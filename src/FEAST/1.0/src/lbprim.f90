...
!!!!!!!!!!!!!!!!!!!
  ...
  if (m==1) then
     ...
     CALL DGBALU2(n,n,KL,KU,A,LDA,norm,nzero,info2)
     if (info2<0) THEN
        info=info2+1
...
!!!! we can decompose here into 4 multiplication
           ...
           if ((klr>0).and.(kur>0)) call DGEMM('N','N',klr,kur,m,-DONE,A(ku+1+m,jmin),kl+ku,A(ku+1-m,jmin+m),kl+ku,DONE,A(ku+1,jmin+m),kl+ku)
...
!!!!!!!!!!!!!!!!!!!
  ...
  if (m==1) then
     ...
     CALL ZGBALU2(n,n,KL,KU,A,LDA,norm,nzero,info2)
     if (info2<0) THEN
        info=info2+1
...
!!!! we can decompose here into 4 multiplication
           ...
           ! M1 (rectangular klr*m) * K1 (rectangular m*kur)
           if ((klr>0).and.(kur>0)) call ZGEMM('N','N',klr,kur,m,-ONEC,A(ku+1+m,jmin),kl+ku,A(ku+1-m,jmin+m),kl+ku,ONEC,A(ku+1,jmin+m),kl+ku)
           ! M1 (rectangular klr*m) * K1 (lower triangular m*m)
           if (klr>0) then
...
!!!!!!!!!!!!!!!!
  ...
  DO  J = 1,MIN(M,N)
     if (abs(A(KU+1,J))<=pivot) then
        ...
        A(KU+1, J)=A(KU+1,J)+sign(nzero,abs(A(KU+1,J)))*norm
        info=info+1
     end if
     KM=min(KL,M-J)
     JU=max(JU,min(J+KU,N))
...
!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  character(len=1) :: TRANSA,TRANSB,SIDE,UPLO2,DIAG2
  integer :: k,ijinit,i
  integer :: m,m1,max
...
!!!!!!!!!!!!!!
  ...
  integer :: info_alloc
  logical :: low,up,full
  integer :: bl, i , k , tmp,  j,imin,jmin,iminb,iminc,imin2,jmin2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do k=1,bl-1
     ...
     call DTRMM(SIDE,UPLO2,TRANSA,DIAG,kd,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
     do i=1,rhs
...
!!!! final block (triangular+rectangular)
  if (tmp/=0) then
     ...
     call DTRMM(SIDE,UPLO2,TRANSA,DIAG,tmp,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do k=1,bl-1
     ...
     call ZTRMM(SIDE,UPLO2,TRANSA,DIAG,kd,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
     do i=1,rhs
...
!!!! final block (triangular+rectangular)
  if (tmp/=0) then
     ...
     call ZTRMM(SIDE,UPLO2,TRANSA,DIAG,tmp,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
...
!!!!!!!!!!!!!!!!!!!
  ...
  if (m==1) then
     ...
     CALL SGBALU2(n,n,KL,KU,A,LDA,norm,nzero,info2)
     if (info2<0) THEN
        info=info2+1
        if (INFO.NE.0) CALL XERBLA( 'DGBALU', -INFO )
        RETURN
...
!!!! we can decompose here into 4 multiplication
           ...
           ! M1 (rectangular klr*m) * K1 (rectangular m*kur)
           if ((klr>0).and.(kur>0)) call SGEMM('N','N',klr,kur,m,-SONE,A(ku+1+m,jmin),kl+ku,A(ku+1-m,jmin+m),kl+ku,SONE,A(ku+1,jmin+m),kl+ku)
           ! M1 (rectangular klr*m) * K1 (lower triangular m*m)
           if (klr>0) then
...
!!!!!!!!!!!!!!!!!!!
  ...
  if (m==1) then
     ...
     CALL CGBALU2(n,n,KL,KU,A,LDA,norm,nzero,info2)
     if (info2<0) THEN
        info=info2+1
        if (INFO.NE.0) CALL XERBLA( 'ZGBALU', -INFO )
        RETURN
...
!!!! we can decompose here into 4 multiplication
           ...
           ! M1 (rectangular klr*m) * K1 (rectangular m*kur)
           if ((klr>0).and.(kur>0)) call CGEMM('N','N',klr,kur,m,-ONEC,A(ku+1+m,jmin),kl+ku,A(ku+1-m,jmin+m),kl+ku,ONEC,A(ku+1,jmin+m),kl+ku)
           ! M1 (rectangular klr*m) * K1 (lower triangular m*m)
           if (klr>0) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do k=1,bl-1
     ...
     call STRMM(SIDE,UPLO2,TRANSA,DIAG,kd,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
     do i=1,rhs
...
!!!! final block (triangular+rectangular)
  if (tmp/=0) then
     ...
     call STRMM(SIDE,UPLO2,TRANSA,DIAG,tmp,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do k=1,bl-1
     ...
     call CTRMM(SIDE,UPLO2,TRANSA,DIAG,kd,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
     do i=1,rhs
...
!!!! final block (triangular+rectangular)
  if (tmp/=0) then
     ...
     call CTRMM(SIDE,UPLO2,TRANSA,DIAG,tmp,rhs,alpha,A(imin,jmin),LDA-1,helper(1,1),kd)
