...
subroutine zdaddcsr(N,opt,alpha,sa,isa,jsa,beta,sb,isb,jsb,sc,isc,jsc)
  ...
  !  C=alpha*A+beta*B 
  !
  !  A is complex double precision, B and C are real double precision, (alpha,beta are COMPLEX)
  !  
  !  Arguments
  !  =========
  !
...
!!! nullify diagonal
  ...
  !! add the two matrices to form the full matrix
  opt=0
  call daddcsr(N,opt,SONE,a,ia,ja,SONE,sb,sib,sjb,sa,sia,sja)
