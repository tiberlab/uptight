...
int main(int argc,char *name[200]) {
  ...
  buff = NULL;
  getline(&buff, &len, fp);
  feastparam[9] = atoi(buff); //omp (number of threads)
  ...
  *(isa)=1;
  ...
  if ((PRE=='s')&&(TYPE=='s')){// case("ssst")
    ...
    sfeast_sst(&UPLO,&N,ssa,isa,jsa,&feastparam,&sepsout,&loop,&sEmin,&sEmax,&M0,sE,sX,&M,sres,&info);
  }
  if ((PRE=='s')&&(TYPE=='g')){// case("ssge")
    sE=calloc(M0,sizeof(float));  // eigenvalues
    sres=calloc(M0,sizeof(float));// residual 
    sX=calloc(N*M0,sizeof(float)); // eigenvectors
    sfeast_sge(&UPLO,&N,ssa,isa,jsa,ssb,isb,jsb,&feastparam,&sepsout,&loop,&sEmin,&sEmax,&M0,sE,sX,&M,sres,&info);
  } 
  ...
  if ((PRE=='d')&&(TYPE=='s')){// case("dsst")
    ...
    dfeast_sst(&UPLO,&N,dsa,isa,jsa,&feastparam,&depsout,&loop,&dEmin,&dEmax,&M0,dE,dX,&M,dres,&info);
  }
  if ((PRE=='d')&&(TYPE=='g')){// case("dsge")
    dE=calloc(M0,sizeof(double));  // eigenvalues
    dres=calloc(M0,sizeof(double));// residual 
    dX=calloc(N*M0,sizeof(double)); // eigenvectors
    dfeast_sge(&UPLO,&N,dsa,isa,jsa,dsb,isb,jsb,&feastparam,&depsout,&loop,&dEmin,&dEmax,&M0,dE,dX,&M,dres,&info);
  }
  ...
  if ((PRE=='c')&&(TYPE=='s')){// case("csst")
    ...
    cfeast_sst(&UPLO,&N,csa,isa,jsa,&feastparam,&sepsout,&loop,&sEmin,&sEmax,&M0,sE,cX,&M,sres,&info);
  }
  if ((PRE=='c')&&(TYPE=='g')){// case("csge")
    sE=calloc(M0,sizeof(float));  // eigenvalues
    sres=calloc(M0,sizeof(float));// residual 
    cX=calloc(N*M0,sizeof(float)); // eigenvectors
    cfeast_sge(&UPLO,&N,csa,isa,jsa,csb,isb,jsb,&feastparam,&sepsout,&loop,&sEmin,&sEmax,&M0,sE,cX,&M,sres,&info);
  }
  ...
  if ((PRE=='z')&&(TYPE=='s')){// case("dsst")
    ...
    zfeast_sst(&UPLO,&N,zsa,isa,jsa,&feastparam,&depsout,&loop,&dEmin,&dEmax,&M0,dE,zX,&M,dres,&info);
  }
  if ((PRE=='z')&&(TYPE=='g')){// case("dsge")
    dE=calloc(M0,sizeof(double));  // eigenvalues
    dres=calloc(M0,sizeof(double));// residual 
    zX=calloc(2*N*M0,sizeof(double)); // eigenvectors
    zfeast_sge(&UPLO,&N,zsa,isa,jsa,zsb,isb,jsb,&feastparam,&depsout,&loop,&dEmin,&dEmax,&M0,dE,zX,&M,dres,&info);
  }
