...
!!!!!!! where A and B if any, are provided by user in CSR format !!!!!!!!!                         
!!!!!!! by Eric Polizzi- 2009  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!! DRIVER_FEAST_SPARSE input file  
  ...
  read(10,*) feastparam(10) !omp (number of threads)
  close(10)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  print *,'matrix -csrfile- size',n
  print *,'sparse matrix A- nnz',nnza,isa(n+1)-1
  if (TYPE=='g') print *,'sparse matrix B- nnz',nnzb,isb(n+1)-1
...
!!!!!!!!!!!!!  FEAST
  ...
  if ((PRE=='s').and.(TYPE=='g')) then ! case("ssge")
     ...
     call feast(UPLO,N,ssa,isa,jsa,ssb,isb,jsb,feastparam,sepsout,loop,sEmin,sEmax,M0,sE,sX,M,sres,info)

  elseif ((PRE=='s').and.(TYPE=='s')) then  !case("ssst")
     allocate(sE(1:M0))     ! Eigenvalue
     allocate(sres(1:M0))   ! Residual 
     allocate(sX(1:n,1:M0))
     call feast(UPLO,N,ssa,isa,jsa,feastparam,sepsout,loop,sEmin,sEmax,M0,sE,sX,M,sres,info)

  elseif ((PRE=='d').and.(TYPE=='g')) then ! case("dsge")
     allocate(dE(1:M0))     ! Eigenvalue
     allocate(dres(1:M0))   ! Residual  
     allocate(dX(1:n,1:M0))
     call feast(UPLO,N,dsa,isa,jsa,dsb,isa,jsa,feastparam,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info)
  ...
  elseif ((PRE=='d').and.(TYPE=='s')) then! case("dsst")
     ...
     allocate(dres(1:M0))   ! Residual 
     allocate(dX(1:n,1:M0))
     call feast(UPLO,N,dsa,isa,jsa,feastparam,depsout,loop,dEmin,dEmax,M0,dE,dX,M,dres,info)
  ...
  elseif ((PRE=='c').and.(TYPE=='g')) then !case("csge")
     ...
     call feast(UPLO,N,csa,isa,jsa,csb,isb,jsb,feastparam,sepsout,loop,sEmin,sEmax,M0,sE,cX,M,sres,info)
  ...
  elseif ((PRE=='c').and.(TYPE=='s')) then ! case("csst")
     ...
     allocate(sres(1:M0))   ! Residual 
     allocate(cX(1:n,1:M0))
     call feast(UPLO,N,csa,isa,jsa,feastparam,sepsout,loop,sEmin,sEmax,M0,sE,cX,M,sres,info)
  ...
  elseif ((PRE=='z').and.(TYPE=='g')) then ! case("zsge")
     ...
     call feast(UPLO,N,zsa,isa,jsa,zsb,isb,jsb,feastparam,depsout,loop,dEmin,dEmax,M0,dE,zX,M,dres,info)
  ...
  elseif ((PRE=='z').and.(TYPE=='s')) then ! case("zsst")
     ...
     allocate(dres(1:M0))   ! Residual 
     allocate(zX(1:n,1:M0))
     call feast(UPLO,N,zsa,isa,jsa,feastparam,depsout,loop,dEmin,dEmax,M0,dE,zX,M,dres,info)
