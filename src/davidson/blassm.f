...
c     aplb1  :   computes   C = A + B               [ A, B, C sorted ] c
c     aplsb  :   computes   C = A + s * B                              c
c     aplsb1 :   computes   C = A + s * B           [ A, B, C sorted ] c
...
c     aplsca :   computes   C = A + s * I            ( s = scalar )    c 
...
c_______________________________________________________________________
      ...
      INTEGER ja( * ), jb( * ), jc( * )
      INTEGER ia( nrow + 1 ), ib( * ), ic( * ), iw( ncol )
...
c
c     When job = 0, only the structure ( i.e. the arrays jc, ic )
c     is computed and the real values are ignored.
c
c     a, ja, ia  =  Matrix A in Compressed Sparse Row format.
c     b, jb, ib  =  Matrix B in Compressed Sparse Row format.
c
...
c     ----------
c
c     c, jc, ic  =  Matrix C in Compressed Sparse Row format.
c           
c     ierr  = integer. serving as error message. 
c
c     ierr = 0 means normal return
c     ierr > 0 means that amub stopped while computing the
...
c_______________________________________________________________________
      ...
      DOUBLE PRECISION a(*), b(*), c(*) 
      INTEGER ja( * ), jb( * ), jc( * ), iw( ncol )
      INTEGER ia( nrow + 1 ), ib( nrow + 1 ), ic( nrow + 1 )
...
c----------------------------------------------------------------------- 
      SUBROUTINE aplb1_complex(nrow,a,ja,ia,b,jb,ib,c,jc,ic,nzmax)
      !DOUBLE PRECISION a(*), b(*), c(*)
      COMPLEX(KIND(0.0d0)) :: a(*), b(*), c(*) 
      INTEGER ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1)
c-----------------------------------------------------------------------
c performs the matrix sum  C = A+B for matrices in sorted CSR format.
c the difference with aplb  is that the resulting matrix is such that
...
c     
 ...
 999  ierr = i 
      !birner
      if (ierr .gt. 0) then
         print *,'aplb1_complex stopped'
         print *,'because the number of elements in C exceeds nzmax'
         stop
      end if
      return
c------------end-of-aplb1_complex----------------------------------------------- 
      end
c----------------------------------------------------------------------- 
