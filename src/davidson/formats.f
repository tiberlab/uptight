...
c           column format (transposition)
c csrcsc_complex  : converts compressed sparse row format to compressed sparse c
c     column format (hermitian transposition)                              c
c           written by birner
...
c-----------------------------------------------------------------------
      subroutine msrcsr (n,ao,jao,iao,wk,iwk)
      !birner added:
      USE sparse_matrix,only:sa,ija
                                !double precision a(*),ao(*),wk(n)
                                !double precision ao(*),wk(n)
                                !INTEGER n ! added by S.Birner
      COMPLEX(KIND(0.0d0)) :: ao(*),wk(n)
                                !integer ja(*),jao(*),iao(n+1),iwk(n+1)
      integer jao(*),iao(n+1),iwk(n+1)
c----------------------------------------------------------------------- 
...
c----------------------------------------------------------------------- 
      subroutine csrcsc_complex (n,a,ja,ia,ao,jao,iao)
      integer ia(n+1),iao(n+1),ja(*),jao(*)
      COMPLEX(KIND(0.0d0))  a(*),ao(*)
c-----------------------------------------------------------------------
c birner
c transposes matrix (into complex conjugate) but only strictly upper part
c the diagonal will be set to zero
c
c Compressed Sparse Row     to      Compressed Sparse Column
c
c (transposition operation)   Not in place. 
...
c--------------- now do the actual copying ----------------------------- 
      do 6 i=1,n
         do 62 k=ia(i),ia(i+1)-1 
            j = ja(k) 
            next = iao(j)
                                !birner take complex conjugate of a(k)
                                !if (job .eq. 1)  then
            if (i .NE. j) then
               ao(next) = conjg(a(k))
            else 
...
c-----------------------------------------------------------------------
      ...
      double precision  a(nnz),ao(lda,ncmax)
c-----------------------------------------------------------------------
