...
c zlhsrcsr : converts hermitian sparse row L to compressed sparse row   c
c zuhsrcsr : converts hermitian sparse row U to compressed sparse row   c
...
c----------------------------------------------------------------------- 
      ...
      subroutine zlhsrcsr (nrow,a,ja,ia,nzmax,ao,jao,iao,indu,ierr)
      integer ia(nrow+1),iao(nrow+1),ja(*),jao(nzmax),indu(nrow+1)
      complex*16 a(*),ao(nzmax)
c-----------------------------------------------------------------------
c Hermitian Sparse Row  to (regular) Compressed Sparse Row (unsorted)
...
c compressed sparse row format. the algorithm is in place (i.e. result 
c may be overwritten onto the input matrix a, ja, ia ----- ). 
c the output matrix delivered by zlhsrcsr is such that each row starts with
c the elements of the lower part followed by those of the upper part.
c----------------------------------------------------------------------- 
c on entry:
c--------- 
...
c work arrays:
c------------
c wk       = complex work array of length n
c iwk      = integer work array of length n+1
c
c notes:
