...
MODULE cpa_multi_types
  ...
  PUBLIC :: real_element, pair_hopping, cpa_multi_params
  ...
  ! TYPE pair_hopping
  !
  ! Off-diagonal-disorder (ODD) hopping for one species pair (Q on sl1,
  ! Q' on sl2), under the BEB scheme (Blackman 1971; Papaconstantopoulos
  ! 1989 multiband generalization; Koepernik 1997/1998 multi-sublattice
  ! generalization). Unlike hopping_vca (single averaged value for all
  ! bonds), each pair keeps its OWN Slater-Koster hopping parameters, read
  ! from that pair's own binary .etb file, evaluated at dist_vegard.
  ! Only used when cpa_multi_params%odd_hopping = .TRUE.
  !===========================================================================
  ...
  TYPE cpa_multi_params
    ...
    ! Used when odd_hopping = .FALSE. (default, backward compatible).
    REAL(dp), ALLOCATABLE :: hopping_vca(:)

    ! Off-diagonal disorder (ODD) for hopping (BEB scheme). .FALSE. (default)
    ! keeps the existing VCA-hopping behavior via hopping_vca(:) above.
    ! .TRUE. uses pairs(:) below instead, one entry per (species on sl1,
...
CONTAINS
  ...
  SUBROUTINE destroy_cpa_multi_params( params )
    ...
    IF ( ALLOCATED(params%pairs) ) THEN
      DO idx = 1, SIZE(params%pairs)
        IF ( ALLOCATED(params%pairs(idx)%t) ) &
          DEALLOCATE(params%pairs(idx)%t)
      END DO
