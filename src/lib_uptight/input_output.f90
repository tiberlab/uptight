...
!
!   Changelog:
!       2014-08-28: adapted open_file to use streaming I/O (M. Auf der Maur)
!
!=============================================================================
!
...
MODULE input_output
  ...
  PUBLIC :: open_file, label_in_file, int_in_file, flag_in_file
  PUBLIC :: int_format, int_to_char
  !===========================================================================
...
CONTAINS
  ...
  !     format_flag, output_flag, replace_flag, stream_flag )
  !
  !===========================================================================
  !
  ...
  !   --> stream_flag  : TRUE => streaming I/O  / FALSE => sequential I/O
  ...
  SUBROUTINE open_file( file_name, file_num, operation, &
    ...
    LOGICAL, INTENT( IN ), OPTIONAL :: format_flag, replace_flag, output_flag, stream_flag
    ...
    CHARACTER ( LEN = : ), ALLOCATABLE :: formatting, access_mode, write_format
    ...
    IF ( PRESENT( stream_flag ).AND. stream_flag .EQV. .TRUE. ) THEN
      access_mode = 'STREAM'
      formatting =  'UNFORMATTED'
    END IF

    IF ( PRESENT( format_flag ) .AND. format_flag .EQV. .FALSE. ) &
      formatting = 'UNFORMATTED'
    ...
    SELECT CASE ( TRIM( operation ) )
       ...
       INQUIRE( FILE = file_name, EXIST = exist_test )    
       IF(.NOT.exist_test) &
         WRITE ( *, '( /, "ERROR : file ''", a, "'' does not exist." )' ) &
         TRIM( file_name )       
          
       OPEN( UNIT = file_num, FILE = file_name, ACCESS = access_mode, &
            FORM = formatting, STATUS = 'OLD', IOSTAT = err )
       ...
       IF ( replace ) THEN
          ...
          OPEN( UNIT = file_num, FILE = file_name, STATUS = 'REPLACE', &
               ACCESS = access_mode, FORM = formatting, IOSTAT = err )
...
20  IF ( TRIM( list ) .NE. TRIM( label ) ) error = .TRUE.
  ...
  FUNCTION int_in_file( file_name, file_num, label )
    ...
    IF ( err .NE. 0 ) THEN
       ...
       call throw_init_exception(ERR_DB_NOINT)
  ...
  FUNCTION flag_in_file( file_name, file_num, label )
    ...
    IF ( err .NE. 0 ) THEN
       ...
       call throw_init_exception(ERR_DB_NOBOOL)
