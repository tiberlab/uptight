...
CONTAINS
  ...
  SUBROUTINE matvec_sparse( n, xV, bV )
    ...
    COMPLEX( dp ), DIMENSION( : ), INTENT( IN )  :: xV
    COMPLEX( dp ), DIMENSION( : ), INTENT( OUT ) :: bV
...
CONTAINS
  ...
  SUBROUTINE msr2msr_full()
    ...
    COMPLEX( dp ), DIMENSION( : ), ALLOCATABLE, TARGET ::  sa2
    INTEGER, DIMENSION( : ), ALLOCATABLE, TARGET        :: ija2
    
    COMPLEX( dp ), DIMENSION( : ), ALLOCATABLE :: sa3, sa4
  ...
  SUBROUTINE print_sparse_matrix_complex( NDIM, filename )
    ...
    COMPLEX( dp ), DIMENSION( NDIM )            :: tmpcV, bcV
    COMPLEX( dp ), DIMENSION( SIZE( ija ) - 1 ) ::  a_csr, ao_csr
    COMPLEX( dp ), DIMENSION( 2 * SIZE( ija ) - 1 - NDIM ) :: ACSR
